# Seventh automated test checks query for ordering by rankUpDown
# asserts right information is added
# test order by rankUpDown function
import sqlite3
...
def test_main():
    ...
    def test_write_dictionary(cursor: sqlite3.Cursor, conn: sqlite3.Connection):
        ...
        # this portion validates that the actual method used in the gui application organizes data by highest
        # rankUpDown value
        data1 = cursor.execute(f'SELECT * FROM test_most_popular_tv_shows ORDER BY rankUpDown DESC LIMIT 1')
        for row in data1:
            a = f'{row[1]}'
            print(a)
            assert a == "123"
        # this portion validates that the opposite method used in the gui application organizes data by lowest
        # rankUpDown value
        data2 = cursor.execute(f'SELECT * FROM test_most_popular_tv_shows ORDER BY rankUpDown LIMIT 1')
        for row in data2:
            b = f'{row[1]}'
            print(b)
            assert b == "-6999"
